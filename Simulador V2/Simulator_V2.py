...
def die_or_recover(chance_of_death, infection_duration):
    """Kills or recover
       Death Conditions:
       - age > 50 years
       - infected"""
    ...
    data['total_death'] = len(death_list)
    for person in death_list: #"kills" the person
        del people[person]
...
def infect(chance_of_infection, distance_to_infection):
    ...
    data['total_infected'] = len(new_infected)
...
def pop_growth():
    ...
    born = []
    for person in people:
        if not(people[person]['infected']) and len(born) == 0 and chance_generator(1):
            born.append(sorted(list(people.keys()))[-1] + 1)
        elif not(people[person]['infected']) and len(born) != 0 and chance_generator(1):
            born.append(born[-1] + 1)
...
def updade_people():
    """Unique loop to upade all people"""
    pass
...
while len(people) > 0:
    ...
    infect(chance_of_death,distance_to_infection)
    time_pass()
    die_or_recover(chance_of_death,infection_duration)
    remove_imunity()
    pop_growth()
    person()
